...
function CryptexRing({ 
  ...
  return (
    ...
    <animated.group 
      ...
      position-z={zPos} // Now driven by the animated zPos from parent
      rotation-y={Math.PI} 
      {...(bind() as any)}
      onPointerOver={(e) => { 
        if (!dragRef.current) {
    ...
    >
      ...
      <group raycast={() => null}>
        {shuffledIcons.map((tex, i) => (
          <CarouselIcon key={i} texture={tex} index={i} activeIndex={activeIndex} radius={iconRadius} baseColor={color} />
        ))}
      </group>
    </animated.group>
  );
...
function InteractiveArtifact({ setHasInteracted }: { setHasInteracted: (val: boolean) => void }) {
  ...
  const [isUnlocked, setIsUnlocked] = useState(false); // NEW: Manage opening state
  ...
  // NEW: Spring for the "Open" spread animation
  const { spread } = useSpring({
    spread: isUnlocked ? 1 : 0,
  ...
  // The final unlock trigger
  const handleUnlock = () => {
    triggerHaptic();
    setIsUnlocked(true);
    setHasInteracted(true); // Signal to the main app that Phase 3 is complete
  };
  ...
  return (
    ...
    >
      ...
      <CenterButton isReady={allRingsTouched} onClick={handleUnlock} />
      
      {/* Rings now have animated zPos based on the spread spring */}
      <CryptexRing 
        ringIndex={1} radiusInner={1.18} radiusOuter={1.55} iconRadius={1.37} 
        zPos={spread.to(v => -0.07 - v * 1.5)} // Slides to -1.57
        dragRef={childIsDraggingRef} onInteract={() => handleRingInteract(1)} color="#00ffff" hitInner={1.15} hitOuter={1.55} 
      />
      <CryptexRing 
        ringIndex={2} radiusInner={0.80} radiusOuter={1.20} iconRadius={0.99} 
        zPos={spread.to(v => -0.05 - v * 1.0)} // Slides to -1.05
        dragRef={childIsDraggingRef} onInteract={() => handleRingInteract(2)} color="#00ffff" hitInner={0.78} hitOuter={1.17} 
      />
      <CryptexRing 
        ringIndex={3} radiusInner={0.40} radiusOuter={0.82} iconRadius={0.60} 
        zPos={spread.to(v => -0.03 - v * 0.5)} // Slides to -0.53
        dragRef={childIsDraggingRef} onInteract={() => handleRingInteract(3)} color="#00ffff" hitInner={0.40} hitOuter={0.805} 
      />
    </group>
  );
}
