...
import { fetchAdminGuests, createAdminGuest, sendAdminInvite, updateGuestGroup } from "./api/client"; // Added imports
...
// --- Helper: Format Activity Kind ---
function formatActivityKind(kind: string | null) {
  if (!kind) return "Unknown";
  // e.g. "rsvp_created" -> "RSVP Created"
...
export default function AdminDashboard() {
  ...
  // 2. Add Sort State (Default: RSVP Descending)
    ...
    return [...filteredGuests].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "group":
          ...
          break;
        case "rsvp":
          // Sort empty/null RSVPs based on direction to keep them together
  ...
  // NEW: Handle Shift+Click Range Selection
  ...
  // NEW: Handle Bulk Group Assignment
  const handleBulkGroupAssign = async (groupLabel: string) => {
    if (selection.selectedIds.length === 0) return;
    const confirmMsg = `Assign ${selection.selectedIds.length} guests to "${groupLabel}"?`;
    if (!window.confirm(confirmMsg)) return;

    try {
      // Use existing endpoint for bulk update
      await fetch('/api/v1/admin/group/bulk', {
        ...
        body: JSON.stringify({ 
          guest_ids: selection.selectedIds, 
  ...
  // Derive unique groups for dropdowns
            ...
            <div className="shrink-0 p-3 sm:p-4 border-b border-primary bg-surface z-20 shadow-sm">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  ...
                  <div className="relative w-full sm:w-64 z-40">
                    ...
                    >
                      ...
                      <ListboxOptions className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-[#0a0a0a] border border-[#45CC2D] py-1 shadow-2xl focus:outline-none">
                        <ListboxOption value="" className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 text-[10px] font-bold uppercase transition-colors ${active ? "bg-[#45CC2D] text-black" : "text-gray-300"}`}>
                          All Groups
                        </ListboxOption>
                        {availableGroups.map((g) => (
                          <ListboxOption key={g} value={g} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 text-[10px] uppercase transition-colors ${active ? "bg-[#45CC2D] text-black" : "text-gray-300"}`}>
                            {({ selected }) => (
                              <>
                                <span className={`block truncate ${selected ? "font-bold" : "font-normal"}`}>{g}</span>
                                {selected && <span className="absolute inset-y-0 left-0 flex items-center pl-3"><CheckIcon className="h-4 w-4" aria-hidden="true" /></span>}
                              </>
                            )}
                          </ListboxOption>
                        ))}
                ...
                <div className="flex items-center gap-2">
                      ...
                      <div className="relative w-40">
                        <Listbox onChange={handleBulkGroupAssign}>
                          ...
                          <ListboxOptions className="absolute right-0 z-50 mt-1 max-h-60 w-64 overflow-auto rounded-md bg-[#0a0a0a] border border-[#45CC2D] py-1 shadow-2xl focus:outline-none">
                            ...
                            {availableGroups.map((g) => (
                              <ListboxOption key={g} value={g} className={({ active }) => `relative cursor-default select-none py-2 pl-4 pr-4 text-[10px] uppercase transition-colors ${active ? "bg-[#45CC2D] text-black" : "text-gray-300"}`}>
                                {g}
                              </ListboxOption>
                            ))}
                          </ListboxOptions>
                  ...
                  <button
                    className="px-2 py-1 border border-primary text-xs hover:bg-[#45CC2D] hover:text-black transition-colors whitespace-nowrap"
                      ...
                      <th className="p-2 w-10 bg-surface">
                        {sortedGuests.length > 0 && (
                          <input
                            ...
                            ref={el => {
                              if (el) {
                                const selectedCount = sortedGuests.filter(g => selection.isSelected(g.id)).length;
                                el.indeterminate = selectedCount > 0 && selectedCount < sortedGuests.length;
                              }
                            }}
                            onChange={() => {
                              const ids = sortedGuests.map((g) => g.id);
                    ...
                    {sortedGuests.map((g, index) => (
                      <tr
                        ...
                        className={`border-b border-primary cursor-pointer hover:bg-neutral-800 ${
                          selection.isSelected(g.id) ? "bg-primary text-surface" : ""
                        }`}
                        onClick={() => setSelectedGuest(g)}
                      >
                        <td
                          onClick={(e) => handleRowClick(e, g.id, index)}
                        ...
                        <td className="p-2 align-top sm:align-middle whitespace-nowrap">
                          {g.rsvps?.status ? (
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium uppercase ${
                              g.rsvps.status === 'attending' 
                                ? (selection.isSelected(g.id) ? 'bg-black text-[#45CC2D] border border-black' : 'bg-[#45CC2D] text-black')
                                : (selection.isSelected(g.id) ? 'bg-black/20 text-black border border-black' : 'bg-neutral-800 text-gray-300 border border-gray-700')
                            }`}>
                              {g.rsvps.status}
                            </span>
                          ) : (
                        ...
                        <td className="p-2 hidden lg:table-cell whitespace-nowrap text-xs">
                          <div className="flex flex-col">
                            <span className="font-bold">{formatActivityKind(g.last_activity_kind || 'Created')}</span>
