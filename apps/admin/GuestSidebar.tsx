  ...
  const timeline = useMemo(() => {
    ...
    sorted.forEach((item) => {
      ...
      const kindInfo = kindMap[item.kind] || {
        Icon: LockClosedIcon,
        label: item.kind || "Unknown Activity",
      };
      ...
      <div className="p-4 flex-1 overflow-auto">
        ...
        <div className="grid grid-cols-1 gap-6 mb-8">
          <div>
            ...
            {guest.invited_at && (
              <p className="text-[10px] text-[#45CC2D]/60 mt-1">LAST SENT: {new Date(guest.invited_at).toLocaleString()}</p>
            )}
          </div>
                ...
                <div key={dayKey} className="relative z-10">
                  ...
                  <div className="flex flex-col gap-2 pl-4">
                    ...
                    {items.map((item) => (
                      <div
                        key={item.__rowId}
                        className="flex items-start gap-3 p-2 border border-[#45CC2D]/30 bg-black"
                      >
                        <div className="pt-0.5">
                          <item.Icon className="h-5 w-5 text-[#9ae68c]" />
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.kind.includes("rsvp") && item.meta?.response && (
                            <p className="text-xs text-[#45CC2D] mt-1 pl-2 border-l border-[#45CC2D]/50">
                              STATUS: {item.meta.response.toUpperCase()}
                            </p>
                          )}
                          
                          {item.kind.includes("email") && item.meta?.subject && (
                            <p className="text-xs text-[#45CC2D]/70 mt-1 truncate">"{item.meta.subject}"</p>
                          )}
