...
import AddGuestSuccessModal from "./AddGuestSuccessModal";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-[400px] rounded-lg bg-black shadow-2xl border border-[#45CC2D] text-white overflow-hidden">
        ...
        <div className="px-5 py-3 border-t border-gray-800 bg-neutral-900/30 flex justify-end gap-3">
          <button onClick={onCancel} className="px-3 py-1.5 text-xs font-bold text-gray-500 hover:text-white uppercase tracking-wider transition-colors">Cancel</button>
          <button onClick={onConfirm} className={`px-4 py-1.5 rounded text-xs font-bold uppercase tracking-wider transition-all text-black ${isDangerous ? 'bg-red-500 hover:bg-red-400' : 'bg-[#45CC2D] hover:bg-[#3bb325]'}`}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
...
export default function GuestList() {
  ...
  const [pendingBulkAssign, setPendingBulkAssign] = useState<any>(null);
  ...
  async function handleAddGuests(guestsToAdd: any[], options: any) {
    ...
    for (const guest of guestsToAdd) {
      ...
      } catch (err: any) { if (err?.message?.includes("409")) existingCount++; }
    }
...
const sortedGuests = useMemo(() => {
  ...
  const initiateBulkAssign = (groupLabel: string) => {
    if (groupLabel === "___CREATE_NEW___") setShowCreateGroupModal(true);
    else setPendingBulkAssign({ groupLabel });
  };

  const executeBulkAssign = async () => {
    try {
      await bulkUpdateGroup(pendingBulkAssign.groupLabel, selection.selectedIds);
      await refreshGuests(true);
      selection.clear();
    } catch (e) { alert("Failed to update groups"); }
  ...
  const handleCreateGroupAndAssign = async (newLabel: string) => {
    ...
    try {
      ...
      await refreshGuests(true);
      selection.clear();
    } catch (e) { alert("Failed to create/assign group"); }
  };
  ...
  const getStatusColorClass = (status: string, isSelected: boolean) => {
    ...
    if (['yes', 'attending', 'accepted'].includes(s)) return isSelected ? 'bg-black text-[#45CC2D] border border-black' : 'bg-[#45CC2D] text-black';
    if (['no', 'declined'].includes(s)) return isSelected ? 'bg-red-900 text-white' : 'bg-red-600 text-white';
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === "a" || e.key === "A") { e.preventDefault(); if (selectedGuest) return; const ids = sortedGuests.map((g) => g.id); if (selection.isAllSelected(ids)) selection.clear(); else selection.selectMany(ids); }
  ...
  const availableGroups = useMemo(() => Array.from(new Set(guests.map(g => canonicalizeGroupLabel(g.group_label)))).filter(g => g !== "—").sort(), [guests]);
  const visibleGroups = useMemo(() => Array.from(new Set(sortedGuests.map(g => canonicalizeGroupLabel(g.group_label)))).filter(g => g !== "—").sort(), [sortedGuests]);
  ...
  return (
    <div className="flex flex-col h-full bg-surface overflow-hidden relative">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full lg:w-auto">
                ...
                <div className="relative w-full sm:w-64 z-40">
                  <Listbox value={filter} onChange={(val) => { setFilter(val as FilterState); setSelectedGroup(null); }}>
                    <ListboxButton className="relative w-full cursor-default border py-1 pl-3 pr-10 text-left text-[10px] uppercase font-bold bg-black text-[#45CC2D] border-[#45CC2D] focus:ring-1 focus:ring-[#45CC2D] hover:bg-[#9ae68c] hover:text-black transition-all">
                    ...
                    <ListboxOptions className="absolute z-50 mt-1 max-h-80 w-full overflow-auto rounded-md bg-[#0a0a0a] border border-[#45CC2D] py-1 shadow-2xl focus:outline-none">
                      ...
                      {["responded_yes", "responded_no", "responded_pending"].map(f => (
                ...
                <div className="relative w-full sm:w-64 z-30">
                  <Listbox value={selectedGroup} onChange={(val) => { if (!val) { setFilter("all"); setSelectedGroup(null); } else { setSelectedGroup(val); } }}>
                    <ListboxButton className="relative w-full cursor-default border py-1 pl-3 pr-10 text-left text-[10px] uppercase font-bold bg-black text-[#45CC2D] border-[#45CC2D] focus:ring-1 focus:ring-[#45CC2D] hover:bg-[#9ae68c] hover:text-black transition-all">
                    ...
                    <ListboxOptions className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-[#0a0a0a] border border-[#45CC2D] py-1 shadow-2xl focus:outline-none">
                      ...
                      {availableGroups.map((g) => (
                        <ListboxOption key={g} value={g} className={({ active }) => `relative cursor-default select-none py-2 pl-8 pr-4 text-[10px] uppercase transition-colors ${active ? "bg-[#45CC2D] text-black" : "text-gray-300"}`}>
                          {({ selected }) => (
                            <><span className={`block truncate ${selected ? "font-bold" : "font-normal"}`}>{g}</span>{selected && <span className="absolute inset-y-0 left-0 flex items-center pl-3"><CheckIcon className="h-4 w-4 text-[#45CC2D]" /></span>}</>
                          )}
                        </ListboxOption>
                      ))}
                    </ListboxOptions>
                ...
                <div className="relative w-full sm:w-48 z-20">
                  <Listbox value={null} onChange={(val: any) => {
                    ...
                    <ListboxButton className="relative w-full cursor-default border py-1 pl-3 pr-10 text-left text-[10px] uppercase font-bold bg-[#45CC2D] text-black border-[#45CC2D] transition-all hover:bg-[#9ae68c]">
                      <span className="block truncate font-bold">{selection.selectedIds.length > 0 ? `(${selection.selectedIds.length}) SELECTED` : "SELECT..."}</span>
                ...
                {selection.selectedIds.length > 0 && (
                  <div className="flex items-center gap-2 mr-2">
                    ...
                    <BulkActions 
                      ...
                      selectedGuests={guests.filter(g => selection.isSelected(g.id))} 
                  ...
                  <tr className="text-left border-b border-primary uppercase text-sm">
                    <th className="p-2 w-10 bg-surface">
                      {sortedGuests.length > 0 && (
                        <input type="checkbox" disabled={!!selectedGuest} className={`h-5 w-5 rounded bg-black border-[#45CC2D] text-[#45CC2D] focus:ring-[#45CC2D] focus:ring-offset-black ${selectedGuest ? "opacity-30 cursor-not-allowed" : ""}`} checked={sortedGuests.length > 0 && sortedGuests.every(g => selection.isSelected(g.id))} onChange={() => { const ids = sortedGuests.map((g) => g.id); if (selection.isAllSelected(ids)) selection.deselectMany(ids); else selection.selectMany(ids); }} />
                      )}
                    </th>
        ...
        {selectedGuest && (
          <div className="absolute inset-y-0 right-0 z-50 flex justify-end w-full lg:static lg:w-auto lg:h-full lg:z-auto bg-black/40 lg:bg-black transition-all duration-300">
            ...
            <div className="relative w-full max-w-[420px] h-full bg-black shadow-2xl lg:shadow-none lg:border-l lg:border-[#45CC2D] overflow-hidden">
              <GuestSidebar guest={selectedGuest} onClose={() => setSelectedGuest(null)} onUpdate={() => refreshGuests(true)} />
            </div>
          </div>
        )}
      </div>
      ...
      <ConfirmationModal isOpen={!!pendingBulkAssign} title="Assign Group" message={`Are you sure you want to assign ${selection.selectedIds.length} selected guests to "${pendingBulkAssign?.groupLabel}"?`} onConfirm={executeBulkAssign} onCancel={() => setPendingBulkAssign(null)} />
    </div>
  );
}